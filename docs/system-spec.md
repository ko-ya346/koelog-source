...
- 認証、workspace、membership、共通認可処理は未実装。
...
- `id`: uuid primary key
- `name`: text not null
- `type`: text not null check in (`personal`, `team`)
...
- `personal_owner_user_id` is not null when `type = 'personal'`
...
- (`workspace_id`, `category`, `occurred_at` desc)
